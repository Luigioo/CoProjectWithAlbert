...
public class GameArena extends JFrame implements Runnable, KeyListener
{
	...
	public void setBricks(Brick[][] brks){
		bricks = brks;
	}
	
	public void setPll(Pll p){
		player = p;
	}
	...
	public void paint (Graphics gr)
	{
		...
		synchronized (this)
		{
			if (!this.exiting)
			{
				...
				if(player != null){
					g.setColor(this.getColourFromString(player.getColour()));
					g.fillRect((int)player.getXPosition(), (int)player.getYPosition(), (int)player.getWidth(), (int)player.getHeight());
				}
			}
